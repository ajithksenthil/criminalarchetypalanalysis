...
def find_optimal_k_for_conditional_analysis(embeddings, criminal_sequences, type2_df, k_range=None, min_effect_size=0.1):
    """
    ...
    for k in k_range:
        ...
        global_matrix = build_conditional_markov(list(test_sequences.keys()), test_sequences, k)
        global_stationary = compute_stationary_distribution(global_matrix)
        ...
        for heading in type2_df["Heading"].unique():
            condition_map = get_condition_map(type2_df, heading)
            ...
            for val in unique_values:
                ...
                cond_matrix = build_conditional_markov(selected_ids, test_sequences, k)
                cond_stationary = compute_stationary_distribution(cond_matrix)
        ...
        if total_effects > 0:
            ...
            print(f"  k={k}: {significant_effects}/{total_effects} significant ({significance_rate:.2%}), "
                  f"mean effect={mean_effect_size:.3f}, score={score:.3f}")
...
def multi_objective_k_selection(embeddings, criminal_sequences, type2_df, k_range=None):
    """
    ...
    for k in k_range:
        ...
        silhouette = sklearn_silhouette_score(embeddings, labels)

        # Conditional effect metrics
        _, effect_results = find_optimal_k_for_conditional_analysis(
...
def main():
    ...
    # -------------------------------
    # Load Type 2 Data Early for Conditional Optimization
    # -------------------------------
    type2_df = None
    if args.auto_k:
        if args.match_only:
    ...
    # Create temporary criminal sequences for k optimization
    criminal_sequences_temp = {}
    event_idx = 0
    for crim_id, data in criminals_data.items():
        criminal_sequences_temp[crim_id] = []
        for event in data["events"]:
            criminal_sequences_temp[crim_id].append(event_idx)  # Temporary placeholder
            event_idx += 1

    # -------------------------------
    # Conditional Effect-Optimized K Selection
    # -------------------------------
    if args.auto_k and type2_df is not None:
        print("\n[INFO] Optimizing k for conditional effect detection...")
        optimal_k, effect_metrics = find_optimal_k_for_conditional_analysis(
            embeddings, criminal_sequences_temp, type2_df
        )
        args.n_clusters = optimal_k
        print(f"[INFO] Selected k={optimal_k} for maximum conditional effects")

        # Save optimization results
        with open(os.path.join(args.output_dir, "k_optimization_results.json"), "w") as f:
            json.dump(effect_metrics, f, indent=4, default=numpy_to_python_default)
    elif args.auto_k:
    ...
    # -------------------------------
    if type2_df is None:  # Only load if not already loaded for k optimization
        if args.match_only:
